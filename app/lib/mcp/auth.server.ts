...
function unauthorized(description: string): Response {
  return Response.json(
    ...
    { status: 401, headers: { "WWW-Authenticate": `Bearer realm="reedright", error="invalid_token", error_description="${description}", resource_metadata="${env.APP_URL}/.well-known/oauth-protected-resource/mcp"` } },
  );
}
